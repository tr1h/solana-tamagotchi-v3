// 🔄 Синхронизация NFT Mints с Leaderboard
// Этот скрипт исправляет рассинхронизацию между таблицами nft_mints и leaderboard

const InlineSyncScript = {
    async syncNFTsWithLeaderboard() {
    ...
    async showSyncStats() {
        try {
            ...
            const { data: mints } = await supabase.from('nft_mints').select('*', { count: 'exact' });
            const { data: players } = await supabase.from('leaderboard').select('*', { count: 'exact' });
            
            console.log('📊 CURRENT STATS:');
            console.log(`- NFT Mints: ${mints?.length || 0}`);
            console.log(`- Players: ${players?.length || 0}`);
            console.log(`- Difference: ${(mints?.length || 0) - (players?.length || 0)}`);
        } catch (error) {
...
window.InlineSyncScript = InlineSyncScript;

console.log('🔄 NFT-Leaderboard Sync Script loaded!');
console.log('Usage:');
